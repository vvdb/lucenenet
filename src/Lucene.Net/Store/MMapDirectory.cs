...
*/
	...
	public class MMapDirectory : FSDirectory
	{
		...
		internal virtual ByteBuffer[] Map(MMapIndexInput input, FileStream fc, long offset, long length)
		{
			 ...
			 */
			if (length == 0)
				return new[] { new ReadOnlyHeapByteBuffer(BytesRef.EMPTY_BYTES, 0, 0) };

			if (input.memoryMappedFile == null)
			{
#if NETSTANDARD
            input.memoryMappedFile = MemoryMappedFile.CreateFromFile(fc, null, length, MemoryMappedFileAccess.Read, HandleInheritability.Inheritable, false);
#else
				input.memoryMappedFile = MemoryMappedFile.CreateFromFile(fc, null, length, MemoryMappedFileAccess.Read, null, HandleInheritability.Inheritable, false);
#endif
			}
